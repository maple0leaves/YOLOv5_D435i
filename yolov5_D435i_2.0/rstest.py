...
#用于获取帧，获取对齐帧之后可以获取depth帧，color帧，深度信息等
def get_aligned_images():
    ...
    depth_image = np.asanyarray(aligned_depth_frame.get_data())  # 深度图（默认16位），这个是要返回的帧
...
class YoloV5:
    ...
    def detect(self, img, canvas=None, view_img=True):
        ...
        if view_img and canvas is None:
            canvas = np.copy(img)
        #将det中的坐标，执信度(概率)，类别id都获取，生成1维列表
        xyxy_list = []
        conf_list = []
        class_id_list = []
        ...
        return canvas, class_id_list, xyxy_list, conf_list,bboxs
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            #cv2.convertScaleAbs()将图片转换成8bit图片
            # depth_colormap = cv2.applyColorMap(cv2.convertScaleAbs(
            #     depth_image, alpha=0.03), cv2.COLORMAP_JET)
            # # Stack both images horizontally
            # images = np.hstack((color_image, depth_colormap))#颜色和深度图片叠加后的图片，没有显示出来
            # #后面使用color_image RGB图片放进model中检测
            
            # Show images

            t_start = time.time()  # 开始计时
            # YoloV5 目标检测
            canvas, class_id_list, xyxy_list, conf_list, bboxs = model.detect(
                color_image)

            t_end = time.time()  # 结束计时\
            #canvas = np.hstack((canvas, depth_colormap))
            #print(class_id_list)
            #camera_xyz_list存放每一个目标在相机中的三维坐标，最后通过坐标计算得到距离
            camera_xyz_list=[]
            if xyxy_list:
                for i in range(len(xyxy_list)):
                    ux = int((xyxy_list[i][0]+xyxy_list[i][2])/2)  # 计算像素坐标系的x
                    uy = int((xyxy_list[i][1]+xyxy_list[i][3])/2)  # 计算像素坐标系的y
                    dis = aligned_depth_frame.get_distance(ux, uy) # 获取目标距离相机的三维距离
                    camera_xyz = rs.rs2_deproject_pixel_to_point(
                        depth_intrin, (ux, uy), dis)  # 计算物体在相机坐标系的xyz
                    camera_xyz = np.round(np.array(camera_xyz), 3)  # 转成3位小数
                    camera_xyz = camera_xyz.tolist()
                    #camera_xyz 在相机坐标系中的坐标，最后cv2.putText显示在图片中
